...
const TrashListBox = () => {
  ...
  // Filter documents with typed parameter
  const filteredDocuments = resumes.filter((doc: Resume) => {
  ...
  return (
    <Popover>
      ...
      <PopoverContent className="bg-background w-[22rem] !px-2" align="end" alignOffset={0} forceMount>
        {isLoading ? (
          ...
          <div className="text-sm">
            ...
            <div className="mt-2 px-1 pb-1">
              {filteredDocuments.length === 0 && (
                <p className="text-xs text-center text-muted-foreground">No documents found</p>
              )}

              {filteredDocuments.map((doc: Resume) => (
                <div
                  key={doc.documentId}
                  role="button"
